...
CURL *easy_handle_init(CURL *curl_handle, RECV_BUF *ptr, const char *url)
{
...
int process_html(CURL *curl_handle, RECV_BUF *p_recv_buf, int *data_type, STACK *stack)
{
    *data_type = HTML;
...
// return 0 if is not png and return 1 if is PNG
// buffer and size of buffer
int is_png(uint8_t *buf, size_t n)
{
    if (n < 8)
    {
        // printf("Byte size error: less than 8 bytes in file header\n");
        return 0;
    }
...
int process_png(CURL *curl_handle, RECV_BUF *p_recv_buf, int *data_type)
{
    // pid_t pid = getpid();
    // char fname[256];
...
int process_data(CURL *curl_handle, RECV_BUF *p_recv_buf, int *data_type, STACK *stack, long *response_code_p)
{
    CURLcode res;
    // char fname[256];
    // pid_t pid = getpid();
    ...
    *response_code_p = response_code;
    ...
    {
        return process_html(curl_handle, p_recv_buf, data_type, stack);
    }
    else if (strstr(ct, CT_PNG))
    {
        return process_png(curl_handle, p_recv_buf, data_type);
    }
    else
    {
...
int curl_add(CURLM *cm, CURL *eh, char* seed_url, RECV_BUF** recv_buf, int* index) {
    char url[256];
    ...
    *recv_buf = malloc(sizeof(RECV_BUF));
    ...
    eh = easy_handle_init(eh, *recv_buf, url);
    curl_easy_setopt(eh, CURLOPT_PRIVATE, (void*)index);
    if (eh == NULL) { 
...
int process_url(CURL *curl_handle, char *seed_url, int *data_type, STACK *stack, long *response_code_p)
{
    CURLcode res;
    char url[256];
    RECV_BUF recv_buf;
    ...
    *response_code_p = 500;
    ...
    strcpy(url, seed_url);
    // printf("process_url: URL is %s\n", url);

    curl_handle = easy_handle_init(curl_handle, &recv_buf, url);
    ...
    process_data(curl_handle, &recv_buf, data_type, stack, response_code_p);
